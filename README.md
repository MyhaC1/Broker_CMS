...
- **Мульти-тенантность (первый шаг)** — коллекция **sites** (slug `^[a-z0-9-]{2,32}$` = идентификатор тенанта для терминала/кабинетов); `GET /v1/cms/brand?site=<slug>` отдаёт бренд конкретного сайта той же контрактной формой cms.brand (404 unknown_site, 400 invalid_site); инвалидация — тегом `cms:brand:<slug>`. Остальные ресурсы per-site — следующий этап.
